...
- **修复后台入口 404（顺带修的健壮性问题）**：访问 `/admin`（末尾没有斜杠）时，未登录跳转用的是相对路径 `login.php`，浏览器会解析到站点根目录的 `/login.php` 而 404。这是**本地 PHP 内置服务器（`php -S`）才会出现的现象**；公网跑 Apache / Nginx 时会先把 `/admin` 规范化成 `/admin/`，**公网本来不受影响**。虽然不影响线上，还是当 bug 修了：改为按当前脚本目录生成绝对路径（`/admin/login.php`），`/admin`、`/admin/`、`/admin/index.php` 均可正常进入，后台「退出登录」跳转同步修正。
- **修复后台菜单跳回前台首页**：同源问题——后台菜单用的是相对链接，从 `/admin`（无尾斜杠）进入时会被解析成站点根目录的 `/index.php?page=xxx`，于是"点什么都回首页"。现给后台页面加 `<base href="/admin/">` 统一基准地址，菜单、退出登录、查看前台等相对链接不再受入口写法影响。
...
- **安装流程与界面**：取消默认 `admin/admin`，安装时自行创建账号；新增安装表单、成功页和已安装提示页，支持手机布局。所有初始数据写入成功后才标记安装完成。
- **新版欢迎内容**：新安装默认生成一篇 `Hello world!`，介绍 Qingmo 和基本使用方式；首条评论来自 Qingmo 团队。已有站点的文章和评论保持原样。
- **登录与密码**：新建及修改密码至少6个字符；同一 IP 在15分钟内登录失败5次后暂时限制尝试。修改密码使其他设备的登录会话失效，HTTPS 下会话 Cookie 启用 Secure。
- **文件存储可靠性**：完整请求锁保护读取、修改和写入，避免并发覆盖；临时文件写完后替换目标文件。保存失败返回错误，不再显示成功，并清理对应 OPcache 缓存。
- **文章与评论修复**：编辑文章保留置顶及插件字段；评论校验长度、主页协议及回复对象。待审核评论不向访客发送提醒，首次审核通过后发送，并在保存后重算评论数。
- **更新与邮件安全**：在线更新及 SMTP 开启证书校验；更新仅使用 HTTPS，检查压缩包路径、解压体积和核心文件，覆盖前备份，复制失败时尝试回滚。
- **自动刷新修复**：网络失败、接口异常或页面隐藏后释放请求状态，后续轮询可继续执行。
- **精简插件**：移除「页脚备案信息」，统一使用「站点设置 → 页脚自定义」，支持 HTML。
- **开发验证**：新增本地开发路由，禁止直接访问数据等内部目录；补充密码、会话、原子保存和12进程并发写入回归测试。

升级说明：保留 `data/` 与 `assets/uploads/`，升级后需重新登录一次，原有账号密码继续有效。使用旧备案插件的用户应先迁移文字到页脚自定义，再停用插件；覆盖更新不会自动删除旧插件文件。服务器需配置有效 CA 证书库。

已知限制：完整请求锁会使慢请求阻塞其他访问；更新回滚尚不能保证断电或进程被强制终止后的自动恢复，更新包签名尚未实现。真实 SMTP 与远程更新仍需部署环境联调。详见 [修复说明](SECURITY-CHANGES.md)。
...
- 后台：`http://你的域名/admin/`
- 账号和密码：安装表单中自行设置，不再创建默认账号。
- 密码至少6个字符，建议组合使用字母、数字和符号；修改密码后，其他已登录设备需要重新登录。
...
2. **使用独立的强密码**：已有安装不会自动修改密码；曾使用默认密码的站点应在后台修改。
