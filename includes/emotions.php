...
function qm_emotion_picker_html() {
    ...
    $html .= '<div class="emoji-bar emotion-panel" id="emotionPanel" style="display:none;">';
    $first = true;
    foreach ($groups as $group) {
        if (!is_array($group) || empty($group['list']) || !is_array($group['list'])) continue;
        $name = (string)($group['groupname'] ?? '');
        if ($name === '') continue;
        if (!$first) $html .= '<span class="sep">·</span>';
        $first = false;
        $html .= '<span class="emotion-group-name" style="font-size:12px;color:#999;margin-right:4px;">' . e($name) . '</span>';
        foreach ($group['list'] as $em) {
            ...
            if ($type === 'sticker') {
                ...
                $html .= '<button type="button" class="em" data-kind="sticker" data-code="' . e($code) . '"' . $ttl . '>'
                    . '<img src="' . e($src) . '" alt=":' . e($code) . ':" style="width:20px;height:20px;vertical-align:middle;pointer-events:none;">'
                    . '</button>';
            } else {
                $text = (string)($em['text'] ?? '');
                if ($text === '') continue;
                $html .= '<button type="button" class="em" data-kind="text" data-val="' . e($text) . '"' . $ttl . '>' . e($text) . '</button>';
            }
        }
        $desc = (string)($group['description'] ?? '');
        if ($desc !== '') {
            $html .= '<span style="font-size:11px;color:#bbb;margin-left:4px;">' . e($desc) . '</span>';
        }
    }
    $html .= '</div>';
    return $html;
